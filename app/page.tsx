...
export default function Home() {
  ...
  const renderScreen = () => {
    ...
    if (screen === "event-detail") return (
      <AppShell screen="events" go={go}><div className="event-detail-page"><button className="back-link" onClick={back}>← Volver a eventos</button><div className="event-detail-hero"><span className="event-date xlarge"><strong>{selectedEvent.date.split(" ")[0]}</strong><small>{selectedEvent.date.split(" ")[1]}</small></span><div><span className="eyebrow">{selectedEvent.area}</span><h1>{selectedEvent.title}</h1><p>Un espacio práctico para hacer una pausa, compartir herramientas sencillas y cuidar el bienestar durante la jornada.</p></div></div><div className="event-detail-grid"><Card><h2>Detalles del evento</h2><dl><div><dt>Fecha</dt><dd>{selectedEvent.day}, {selectedEvent.date.toLowerCase()}</dd></div><div><dt>Hora</dt><dd>{selectedEvent.time}</dd></div><div><dt>Duración</dt><dd>{selectedEvent.duration}</dd></div><div><dt>Lugar</dt><dd>{selectedEvent.place}</dd></div><div><dt>Modalidad</dt><dd>{selectedEvent.mode}</dd></div></dl></Card><Card className="rsvp-card"><span>◎</span><h2>¿Te interesa participar?</h2><p>Confirma tu interés y lo agregaremos a tu agenda personal.</p><Button onClick={() => { setRsvp((items) => items.includes(selectedEvent.id) ? items : [...items, selectedEvent.id]); showToast("Asistencia confirmada"); }}>{rsvp.includes(selectedEvent.id) ? "✓ Asistencia confirmada" : "Confirmar asistencia"}</Button><Button secondary onClick={() => showToast("Evento agregado al calendario")}>Agregar al calendario</Button></Card></div></div></AppShell>
    );
    ...
    if (screen === "notifications") return (
      <AppShell screen="profile" go={go}><PageHeader eyebrow="A tu ritmo" title="Notificaciones" description="Elige los recordatorios que te resulten útiles." /><div className="settings-columns"><Card><h2>Preferencias</h2><label className="toggle-row"><div><strong>Notificaciones</strong><small>Avisos generales de bienestar</small></div><input type="checkbox" checked={notificationsOn} onChange={() => setNotificationsOn(!notificationsOn)} /></label><label className="toggle-row"><div><strong>Recordatorios de pausa</strong><small>Máximo uno durante tu jornada</small></div><input type="checkbox" checked={remindersOn} onChange={() => setRemindersOn(!remindersOn)} /></label><label className="toggle-row"><div><strong>Eventos de mi área</strong><small>Solo actividades relevantes</small></div><input type="checkbox" defaultChecked /></label><div className="quiet-hours"><strong>Horario silencioso</strong><p>Fuera de tu jornada no enviaremos recordatorios.</p><span>18:00 — 07:30</span></div></Card><Card><h2>Ejemplos de avisos</h2>{["Tu pausa de 5 minutos está disponible.", "Hoy hay una actividad de bienestar a las 13:00.", "¿Cómo te sientes hoy?"].map((x, i) => <div className="notification-example" key={x}><span>{["◷", "□", "◡"][i]}</span><div><strong>Bienestar INSN</strong><p>{x}</p><small>{i === 0 ? "Hace 2 min" : i === 1 ? "Hoy, 09:00" : "Ayer"}</small></div></div>)}</Card></div></AppShell>
    );
